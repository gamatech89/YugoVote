    ...
    showResults: function(data) {
      ...
      if (data.results) {
        data.results.forEach(function(result) {
          const $contender = $('.yuv-contender[data-contender-id="' + result.item_id + '"]');
          
          if ($contender.length) {
            // Update result bar
            $contender.find('.yuv-result-bar').css('width', result.percent + '%');
            
            // Update text
            $contender.find('.yuv-result-percent').text(result.percent + '%');
            $contender.find('.yuv-result-votes').text(result.votes + ' glasova');
